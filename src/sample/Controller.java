...
public class Controller {
    ...
    void mathAction(char operation) {
        if(this.operation != '+' && this.operation != '-' && this.operation != '*' && this.operation != '/') {
            this.first_num = Float.parseFloat(this.str_num);        // тут записываем в первую переменную
            label_field.setText(String.valueOf(operation));         // указываем операцию
            this.str_num = "";                                      // очищаем поле для второй переменной
